...
RSpec.describe Codebreaker::Mark do
  subject(:current_subject) { Codebreaker::Mark }
  ...
  context 'when #mark' do
    [
      ...
      it "Secret code is #{item[0]}, guess #{item[1]}, return #{item[2]}" do
        expect(current_subject.call(item[0].split('').map(&:to_i), item[1].split('').map(&:to_i))).to eq(item[2])
      end
    end
  end
end      
